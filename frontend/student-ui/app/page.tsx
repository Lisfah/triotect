...
export default function StudentPage() {
    ...
    if (saved && savedId) {
      try {
        const claims = JSON.parse(
          atob(saved.split(".")[1].replace(/-/g, "+").replace(/_/g, "/")),
        );
  ...
  const startSSE = (oid: string) => {
    ...
    const es = new EventSource(
      `${NOTIFICATION_URL}/notifications/stream/${oid}`,
    );
    esRef.current = es;
      ...
      <main className="container" style={{ paddingTop: "4rem" }}>
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          <p style={{ color: "var(--text-muted)", marginTop: "0.5rem" }}>
            Iftar ordering — fast, reliable, real-time
          </p>
        </div>
        <div
    ...
    >
            ...
            {STATUS_STEPS.map((step) => (
              <div
                ...
                className={`status-step ${
                  orderStatus === step
                    ? "active"
                    : STATUS_STEPS.indexOf(orderStatus as OrderStatus) >
      ...
      <div className="card" style={{ marginTop: "1rem" }}>
        ...
        >
          ...
          <button
            className="btn btn-primary"
            onClick={() => token && fetchHistory(token)}
                  ...
                  >
                    {order.items.map((it, idx) => {
                      ...
                      return (
                        ...
                        >
                          {m ? `${m.emoji} ${m.name}` : it.menu_item_id} ×{" "}
                          {it.quantity}
                        </span>
                      );
                    })}

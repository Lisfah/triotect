...
const NOTIFICATION_URL =
  process.env.NEXT_PUBLIC_NOTIFICATION_URL || "http://localhost:8005";
...
export default function AdminPage() {
    ...
    for (const svc of SERVICES) {
      ...
      try {
        ...
        setServiceStatuses((prev) => ({
          ...prev,
          [svc.id]: {
            status: r.ok ? data.status : "degraded",
  ...
  const toggleChaos = async () => {
    ...
    try {
      ...
      await fetch(`${NOTIFICATION_URL}/notifications/chaos/${endpoint}`, {
        method: "POST",
      });
      await checkChaos();
  ...
  const healthyCount = Object.values(serviceStatuses).filter(
    (s) => s.status === "healthy",
  ).length;
    ...
    >
        ...
        >
          ...
          <div>
            <p style={{ color: "var(--danger)", fontWeight: 700 }}>
              ALERT: Order Gateway Latency Critical!
            </p>
            <p style={{ color: "var(--text-muted)", fontSize: "0.875rem" }}>
              Average response time is {gatewayLatency}ms — exceeds 1000ms
              threshold. System may be under saturation.
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        <div className="card" style={{ textAlign: "center" }}>
          <div
            className="metric-value"
            style={{ color: latencyAlert ? "var(--danger)" : "var(--success)" }}
          >
        ...
        <div className="card" style={{ textAlign: "center" }}>
          <div
            className="metric-value"
            style={{
              color: chaosActive ? "var(--danger)" : "var(--text-muted)",
            }}
          >
            {chaosActive ? "ON" : "OFF"}
          </div>
      ...
      >
        ...
        <div className="card">
          ...
          >
            Inject synthetic failures into the Notification Hub to verify
            service isolation. The Order Gateway and Stock Service must remain
            operational.
          </p>
          <div
            style={{
              ...
              borderRadius: "10px",
              background: "rgba(255,255,255,0.03)",
              border: `2px solid ${chaosActive ? "var(--danger)" : "var(--card-border)"}`,
              marginBottom: "1rem",
            }}
          >
            <div
            ...
            >
              <div>
                ...
                >
                  {chaosActive
                    ? "💥 Injecting 503 errors and severing SSE connections"
                    : "Nominal — no chaos active"}
                </p>
              </div>
              <div
                style={{
                  ...
                  height: "32px",
                  borderRadius: "999px",
                  background: chaosActive ? "var(--danger)" : "#333",
                  cursor: chaosLoading ? "not-allowed" : "pointer",
                  position: "relative",
                  transition: "background 0.3s",
                  opacity: chaosLoading ? 0.6 : 1,
                }}
                id="chaos-toggle"
          ...
          {chaosActive && (
            ...
            >
              ...
              >
                Notification Hub is returning 503. Verify that orders still
                process via Gateway + Stock + Kitchen.
              </p>
            </div>
          )}
          ...
          <div style={{ marginTop: "1.5rem" }}>
            ...
            {[
              {
                label: "Gateway avg latency (30s)",
                query:
                  'sum(rate(http_request_duration_seconds_sum{job="order-gateway"}[30s]))/sum(rate(http_request_duration_seconds_count{job="order-gateway"}[30s]))',
              },
              {
                label: "Total orders (1m)",
                query:
                  'sum(increase(http_requests_total{job="order-gateway", status="202"}[1m]))',
              },
              {
                label: "Error rate (5m)",
              ...
              <a
                key={label}
                href={`${PROMETHEUS_URL}/graph?g0.expr=${encodeURIComponent(query)}&g0.tab=0`}
                target="_blank"
                rel="noreferrer"
                style={{
                  display: "block",
      ...
      <div className="card" style={{ marginTop: "1.5rem" }}>
        ...
        >
          Live Grafana metrics — must be logged in to Grafana for the embed to
          display.{" "}
          <a
            href={GRAFANA_URL}
            target="_blank"
